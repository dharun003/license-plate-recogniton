...
def preprocess(img):
	...
	gray = cv2.cvtColor(imgBlurred.astype('float32'), cv2.COLOR_BGR2GRAY)
...
##
##              time.sleep(0.2)                     #required or not??!!
          
#-----------------------------------------------------------------------------------increasing resolution
	'''
	#camera = picamera.PiCamera()
	#camera.capture('testData1/test.jpeg')
...
# Coefficient
	...
	cv2.imwrite('project.jpg',dst)

	imag = cv2.imread("project.jpg")
	threshold_img = preprocess(imag)
